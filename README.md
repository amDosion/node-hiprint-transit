...
[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/amDosion/node-hiprint-transit)
...
```bash
wget http://192.168.50.97/root/node-hiprint-transit/-/raw/main/install.sh
...
本仓库使用 GitLab CI 自动构建文明本地 GitLab Registry 镜像：

- 推送到 `main` / `master`：构建并推送 `latest`，同时生成分支 tag 和短 SHA tag。
...
```bash
git clone git@192.168.50.97:root/node-hiprint-transit.git
cd node-hiprint-transit
```

也可以直接使用文明 GitLab Registry 镜像：
...
当前独立维护版本优先使用 Docker / GitLab Registry 部署；Windows 打包产物以后续 GitLab 发布为准。
...
- **defaultPrinterOnly**: 可手动写入 `config.json`。设置为 `true` 时，中转服务只向 Web 端暴露每个 `electron-hiprint` 客户端的默认打印机；如果客户端没有默认打印机标记，则回退显示该客户端完整打印机列表。

可选环境变量：

- `HIPRINT_CONFIG_PATH`: 指定配置文件路径，默认读取项目根目录的 `config.json`。
- `HIPRINT_LOG_DIR`: 指定日志目录，默认写入 `./logs`。
...
### socket.on("file.export.v1", (options) => {})

向指定 `electron-hiprint` 客户端发送受控文件导出任务。该客户端必须在 `clientInfo.capabilities.fileExport.enabled` 中声明已启用文件导出能力。

`options` 必须包含：

- `client`: 目标 `electron-hiprint` 客户端 socket id。
- `taskId`: 导出任务 id，用于回调匹配。
- `fileName`: 文件名，不能是本机绝对路径。
- `mode`: 当前支持 `binary`。
- `payload`: base64 内容。
- `size`: 解码后的字节数。
...
| node-hiprint-transit     | [gitlab](http://192.168.50.97/root/node-hiprint-transit)                                                               | [registry](http://192.168.50.97/root/node-hiprint-transit/container_registry) | web 与客户端中转服务 Node 实现                                     |
